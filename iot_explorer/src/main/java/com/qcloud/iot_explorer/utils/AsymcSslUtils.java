...
public class AsymcSslUtils {
    ...
    public static SSLSocketFactory getSocketFactory() {
        ...
        try {
            ...
            TXLog.e(TAG, "getSocketFactory失败，创建CertificateFactory错误。", e);
        }
        ...
        // load CA certificate
        {
            Log.d(TAG, "getSocketFactory: ---------------加载证书");
            ByteArrayInputStream caInput = new ByteArrayInputStream(CA.caCrt.getBytes(Charset.forName("UTF-8")));
            parser = new PEMParser(new InputStreamReader(caInput));
            Object object = null;
            try {
                ...
                TXLog.e(TAG, "解析CA失败。", e);
                return null;
            }

            if (!(object instanceof X509CertificateHolder)) {
                TXLog.e(TAG, "CA文件不是X509CertificateHolder。");
                return null;
            }
        ...
        try {
            // CA certificate is used to authenticate server
            Log.d(TAG, "getSocketFactory: ------------CA证书用于验证服务器");
            KeyStore caKs = KeyStore.getInstance(KeyStore.getDefaultType());
            caKs.load(null, null);
            caKs.setCertificateEntry("ca-certificate", caCert);
            TrustManagerFactory tmf = TrustManagerFactory.getInstance(TrustManagerFactory.getDefaultAlgorithm());
            ...
            // finally, create SSL socket factory
            Log.d(TAG, "getSocketFactory: -----------------最后，创建SSL套接字工厂");
            SSLContext context = SSLContext.getInstance("TLS");
